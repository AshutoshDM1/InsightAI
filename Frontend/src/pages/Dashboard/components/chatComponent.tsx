...
export default function ChatComponent() {
  ...
  const { messages, isLoading, sendMessage, abortStream } = useChat();
  ...
  return (
    <div className="relative w-full min-h-screen flex flex-col items-center">
      ...
      <div className="sticky bottom-0 w-full max-w-3xl px-4 pb-8">
        ...
        {messages.length === 0 && (
          <div className="flex items-center justify-center flex-wrap gap-3 mt-6">
            <QuickAction
              ...
              label="Generate Code"
              onClick={() => setMessage("Generate a React component for ")}
            />
            <QuickAction
              icon={<Rocket className="w-4 h-4" />}
              label="Launch App"
              onClick={() => setMessage("Help me build an app that ")}
            />
            <QuickAction
              icon={<Layers className="w-4 h-4" />}
              label="UI Components"
              onClick={() => setMessage("Create a UI component for ")}
            />
            <QuickAction
              icon={<Palette className="w-4 h-4" />}
              label="Theme Ideas"
              onClick={() => setMessage("Suggest color themes for ")}
            />
            <QuickAction
              icon={<CircleUserRound className="w-4 h-4" />}
              label="User Dashboard"
              onClick={() => setMessage("Design a user dashboard with ")}
            />
            <QuickAction
              icon={<MonitorIcon className="w-4 h-4" />}
              label="Landing Page"
              onClick={() => setMessage("Create a landing page for ")}
            />
          </div>
        )}
      </div>
